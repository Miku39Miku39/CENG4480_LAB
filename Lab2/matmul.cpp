...
void matmul_tiling() {
  ...
  // Express the tiling parameters as constexpr to avoid runtime overhead.
  // The block size for the inner loops.
  constexpr int block_i = 64;
  constexpr int block_j = 64;
  constexpr int block_k = 32;
  // The unrolling factors for the inner loops.
  constexpr int MR = 4;
  constexpr int NR = 4;

  // The B tile for the inner loops, aligned to 64 bytes for better cache locality.
  alignas(64) static int B_tile[block_j * block_k]; // using alignas to ensure 64-byte alignment for better cache locality.
...
void matmul_unrolled() {
  // Loop unrolling (factor 4) of the inner k-loop to reduce loop overhead and expose more ILP to the compiler.
  memset(C, 0, sizeof(C));
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      int sum = 0;
